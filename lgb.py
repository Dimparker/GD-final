...
def train_val_split(names,fold):
    ...
    val_names=seqs[fold*val_size:(fold+1)*val_size]
    train_names=seqs[:fold*val_size:]+seqs[(fold+1)*val_size:]
    train_set=[]
    val_set=[]
    for itemTrain in train_names:
        train_set+=[itemTrain+'_'+i for i in seq_frames[itemTrain]]
    for itemVal in val_names:
        val_set+=[itemVal+'_'+i for i in seq_frames[itemVal]]
...
#训练数据
train_val=pd.read_csv('feature/train_val_df.csv')
#featureMJX
train_mjx=pd.read_csv('feature/train_feature.csv',header=None)
test_mjx=pd.read_csv('feature/test_feature.csv',header=None)
...
#time_feature
train_valTime=pd.read_csv('feature/trainValTimeFrt.csv')
testTime=pd.read_csv('feature/testTimeFrt.csv')
#
...
test_dataRaw=pd.read_csv('feature/test_df.csv')
