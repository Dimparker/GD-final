...
class BASE():
    ...
    def loaddata(self, train_dir, batch_size, shuffle,is_train=True):
        ...
        train_sampler = DistributedSampler(image_datasets)
        dataset_loaders = torch.utils.data.DataLoader(image_datasets, batch_size=batch_size, sampler=train_sampler, shuffle=shuffle, num_workers=self.num_workers,  pin_memory=True)
