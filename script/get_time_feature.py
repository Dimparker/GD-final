...
def get_time_frt(annos,write_path):
    ...
    for i in range(len(annos)):
        anno=annos[i]
        imgId = anno['id']
        frame_name = [(k['frame_name'],k['gps_time']) for k in anno['frames']]  # 图片序列
        for name,gps_time in frame_name:
            timeArray = time.localtime(gps_time)
            time_frt['name'].append(imgId + '_' + name)
...
get_time_frt(annos_train,'feature/trainValTimeFrt.csv')
get_time_frt(annos_test,'feature/testTimeFrt.csv')