    ...
    for s in list(df.annotations):
        ...
        for i in range(0,len(frames)-1):
            ...
            if f["frame_name"] == map_key:
                key_time.append(f["gps_time"])
    print(len(jpg_name_1),len(gap_time_1), len(label), len(key_time))
...
# train_df.to_csv('feature/baseline_train_add.csv',index=False)
# test_df.to_csv('feature/baseline_test_add.csv',index=False)
